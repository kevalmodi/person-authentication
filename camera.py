...
def start_app(path,email):
    ...
    facec = cv2.CascadeClassifier('C:/Users/hp/Desktop/Expression20-1/haarcascade_frontalface_alt2.xml')
    font = cv2.FONT_HERSHEY_COMPLEX

    frame_h = int(rgb.get(cv2.CAP_PROP_FRAME_HEIGHT))
    frame_w = int(rgb.get(cv2.CAP_PROP_FRAME_WIDTH))

    video_out="C:/Expression/static/videos/" + 'expression_detected' + '.mp4'  # '.webm'
    # for web support
        ...
        if len(faces) == 0:
            ...
            for frame in data:
                for box in frame:
                    emotion[box] += 1
            dict = emotion
            # print("_+_+_++_+__++_+_+", dict)
            print(ls[0][0])
            for i in range(len(dict)):

                ls[i][0]=(dict[i])

            if ls[4][0] > 5 and "added" not in ls[0] :
                print('---------',ls)
                cv2.imwrite('F:\missbehavedphotoes\{}.jpg'.format(str(len(os.listdir(r"F:\missbehavedphotoes")) + 1)),fr)
                MESSAGE = "misbehave detected"

                fromaddr = "noreplymisbehave@gmail.com"
                ...
                server.login(fromaddr, "Aniketshah@123456")
    ...
    # plt.ylabel('Frame Percentage')
    # plt.title('Video Analysis Graph')
    # plt.savefig('C:/Users/hp/Desktop/Expression20-1/static/images/graph.jpg')
    # plt.show()
    #
    # plt.gcf().clear()
    print("--------------------------",dict)
